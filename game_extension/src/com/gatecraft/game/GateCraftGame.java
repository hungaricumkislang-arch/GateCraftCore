...
public class GateCraftGame extends AndroidNonvisibleComponent implements OnPauseListener, OnResumeListener, OnDestroyListener {
  ...
  private Dialog dialog; private View activeView; private GameMetrics metrics;
  private boolean started,buttonHooked; private int calculationCount,language=2; private boolean testMode=true;
  private int previousOrientation=ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED; private boolean orientationChanged;
  public GateCraftGame(ComponentContainer container){super(container.$form());activity=container.$context();container.$form().registerForOnPause(this);container.$form().registerForOnResume(this);container.$form().registerForOnDestroy(this);scheduleWorkshopButtonHook(0);}
  private void scheduleWorkshopButtonHook(final int attempt){mainHandler.postDelayed(new Runnable(){@Override public void run(){if(buttonHooked)return;if(hookWorkshopButton())return;if(attempt<48)scheduleWorkshopButtonHook(attempt+1);}},attempt==0?0L:250L);}
  private boolean hookWorkshopButton(){try{View root=activity.findViewById(android.R.id.content);final Button b=findWorkshopButton(root);if(b==null)return false;b.setOnClickListener(new View.OnClickListener(){@Override public void onClick(View v){showGameOverlay();}});buttonHooked=true;return true;}catch(Throwable ignored){return false;}}
  private Button findWorkshopButton(View root){if(root==null)return null;if(root instanceof Button){CharSequence cs=((Button)root).getText();if(cs!=null){String s=cs.toString().trim().toLowerCase();if(s.contains("workshop run")||s.equals("workshop"))return (Button)root;}}if(root instanceof ViewGroup){ViewGroup g=(ViewGroup)root;for(int i=0;i<g.getChildCount();i++){Button b=findWorkshopButton(g.getChildAt(i));if(b!=null)return b;}}return null;}
  private void enterLandscape(){try{previousOrientation=activity.getRequestedOrientation();if(previousOrientation!=ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE){orientationChanged=true;activity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);}}catch(Throwable ignored){orientationChanged=false;}}
  ...
  private void closeGameOverlay(boolean dispatchExit){started=false;releaseActive();try{if(dialog!=null&&dialog.isShowing())dialog.dismiss();}catch(Throwable ignored){}dialog=null;restoreOrientation();if(dispatchExit)EventDispatcher.dispatchEvent(this,"ExitRequested");}
  void requestExitFromView(){closeGameOverlay(true);} void reportScore(int score){EventDispatcher.dispatchEvent(this,"ScoreChanged",score);} void reportLevelComplete(int level,int score){EventDispatcher.dispatchEvent(this,"LevelCompleted",level,score);} void reportGameComplete(int score){EventDispatcher.dispatchEvent(this,"GameCompleted",score);} void reportGameOver(int score){EventDispatcher.dispatchEvent(this,"GameOver",score);}
  ...
  @Override public void onPause(){if(metrics!=null)metrics.setPaused(true);} @Override public void onResume(){if(started&&metrics!=null)metrics.setPaused(false);} @Override public void onDestroy(){closeGameOverlay(false);mainHandler.removeCallbacksAndMessages(null);}
}
