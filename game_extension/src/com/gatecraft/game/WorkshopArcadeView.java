...
final class WorkshopArcadeView extends BaseArcadeView {
  ...
  private boolean left,right,jump,dash,ground,wasGround;
  ...
  @Override public void restart(){ps.clear();items.clear();hazards.clear();fx.clear();x=92;y=300;vx=vy=cam=0;ground=wasGround=false;coyote=jumpBuf=dashCd=boost=invuln=0;checkpoint=92;build();invalidate();}
  ...
  @Override void updateGame(float dt){clock+=dt;fx.update(dt);dashCd=Math.max(0,dashCd-dt);boost=Math.max(0,boost-dt);invuln=Math.max(0,invuln-dt);jumpBuf=Math.max(0,jumpBuf-dt);coyote=Math.max(0,coyote-dt);for(Platform q:ps)if(q.moving)q.y=q.base+(float)Math.sin(clock*1.6f+q.phase)*28;
    float vmax=boost>0?390:315,acc=ground?1350:880;if(left)vx-=acc*dt;if(right)vx+=acc*dt;if(!left&&!right)vx*=Math.pow(ground?.0006:.04,dt);vx=Math.max(-vmax,Math.min(vmax,vx));if(dash&&dashCd<=0){dash=false;dashCd=.85f;float d=right?1:left?-1:vx<0?-1:1;vx=d*(boost>0?560:480);fx.dust(x+19,y+56,10,Color.rgb(162,133,89));}
    if(jump)jumpBuf=.12f;if(jumpBuf>0&&(ground||coyote>0)){vy=-535;ground=false;coyote=jumpBuf=0;fx.dust(x+19,y+56,8,Color.rgb(151,145,132));}vy+=1320*dt;float nx=x+vx*dt,ny=y+vy*dt;wasGround=ground;ground=false;for(Platform q:ps)if(x+38>q.x&&x<q.x+q.w&&y+58<=q.y+9&&ny+58>=q.y&&vy>=0){ny=q.y-58;vy=0;ground=true;if(!wasGround)fx.dust(x+19,q.y,6,Color.rgb(126,121,109));}if(wasGround&&!ground)coyote=.10f;x=Math.max(0,Math.min(world-40,nx));y=ny;if(y>535)hurt();for(Hazard h:hazards)if(invuln<=0&&x+38>h.x&&x<h.x+h.w&&y+58>h.y&&y<h.y+28){fx.sparks(x+19,y+42,18,Color.rgb(255,176,54));hurt();break;}
    for(Pickup q:items)if(!q.got&&Math.abs(x+19-q.x)<34&&Math.abs(y+29-q.y)<39){q.got=true;if(q.kind==0)score+=100;else if(q.kind==1){tools++;score+=250;}else{coffee++;boost=4.5f;score+=150;}fx.sparks(q.x,q.y,10,q.kind==2?Color.rgb(255,93,53):Color.rgb(255,211,67));owner.reportScore(score);}if(x>world*.36f&&checkpoint<world*.3f)checkpoint=world*.36f;if(x>world*.68f&&checkpoint<world*.6f)checkpoint=world*.68f;float target=x-(vx>0?310:vx<0?170:250);cam+=(target-cam)*Math.min(1,dt*7.5f);cam=Math.max(0,Math.min(world-830,cam));if(x>world-100){score+=1200+tools*100;level++;owner.reportLevelComplete(level,score);restart();}}
  private void hurt(){if(invuln>0)return;lives--;invuln=1.2f;fx.sparks(x+19,y+30,16,Color.rgb(255,80,50));if(lives<=0){owner.reportGameOver(score);lives=3;score=Math.max(0,score-350);tools=coffee=0;checkpoint=92;}x=checkpoint;y=250;vx=0;vy=-120;cam=Math.max(0,checkpoint-230);}
