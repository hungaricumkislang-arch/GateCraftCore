...
@DesignerComponent(version=6, description="Offline GateCraft MotorDiag controller recognition core with bundled image OCR, UTF-8 12-language TOP3 confirmation UI, and GateCraft MD_INDEX asset loading.", category=ComponentCategory.EXTENSION, nonVisible=true, iconName="")
...
public class GateCraftMotorVision extends AndroidNonvisibleComponent {
  ...
  @SimpleFunction
  public String Version(){return "1.6.0-phase2o";}
  @SimpleFunction
  ...
  public YailList TopMatchDisplayList(){
    ...
    for(MotorVisionMatcherCore.Match m:matcher.lastMatches()){
      ...
      String brand=m.brandId==null?"":m.brandId.toUpperCase(Locale.ROOT);
      out.add((brand.length()==0?"":brand+" · ")+name+" · "+scoreLabel+": "+String.format(Locale.ROOT,"%.2f",m.score));
    }
    return YailList.makeList(out);
  }
