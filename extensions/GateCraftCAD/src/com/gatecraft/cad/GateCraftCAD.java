...
// GateCraftCAD native extension v1
...
@DesignerComponent(version = 1, description = "GateCraft native CAD drawing surface. No MIT App Inventor Canvas component is used.", category = ComponentCategory.EXTENSION, nonVisible = false, iconName = "")
...
public class GateCraftCAD extends AndroidViewComponent {
  ...
  public void Clear() { view.objects.clear(); view.selectedIndex = -1; view.invalidate(); DrawingChanged(view.objects.size()); }

  @SimpleFunction(description = "Delete the selected object.")
  public void DeleteSelected() {
    if (view.selectedIndex >= 0 && view.selectedIndex < view.objects.size()) {
      view.objects.remove(view.selectedIndex); view.selectedIndex = -1; view.invalidate(); DrawingChanged(view.objects.size()); SelectionChanged(0, "");
    }
  }
  ...
  private class CadView extends View {
    ...
    @Override public boolean onTouchEvent(MotionEvent e) {
      ...
      if (e.getAction()==MotionEvent.ACTION_UP && drawing) {
        float[] s=snapPoint(x,y); ex=s[0]; ey=s[1]; CadObject o=new CadObject(nextId++,tool,sx,sy,ex,ey); objects.add(o); drawing=false;
        selectedIndex=objects.size()-1; invalidate(); ObjectCreated(o.id,o.type); SelectionChanged(o.id,o.type); DrawingChanged(objects.size()); return true;
      }
      return true;
    }
    ...
    private float[] snapPoint(float x,float y) {
      ...
      for (int i=0;i<objects.size();i++) {
        CadObject o=objects.get(i); float d1=distance(x,y,o.x1,o.y1); if(d1<best){best=d1;bestX=o.x1;bestY=o.y1;}
        float d2=distance(x,y,o.x2,o.y2); if(d2<best){best=d2;bestX=o.x2;bestY=o.y2;}
      }
    ...
    private int hitTest(float x,float y) {
      ...
      for(int i=objects.size()-1;i>=0;i--){
        ...
        if(o.type.equals("RECT")){
          float l=Math.min(o.x1,o.x2),r=Math.max(o.x1,o.x2),t=Math.min(o.y1,o.y2),b=Math.max(o.y1,o.y2);
          if(distanceToSegment(x,y,l,t,r,t)<=tol||distanceToSegment(x,y,r,t,r,b)<=tol||distanceToSegment(x,y,r,b,l,b)<=tol||distanceToSegment(x,y,l,b,l,t)<=tol) return i;
        }
